/**
 * Regenerate Merrick: new base (no hat, elegant/sexy) + all 15 outfits + 15 expressions.
 * Run: node scripts/regen-merrick-full.mjs
 */
...
const NEG = "low quality, blurry, deformed, extra fingers, bad anatomy, text, watermark, signature, worst quality, ugly, missing arm, missing hand, missing limb, extra limbs, poorly drawn hands, mutation, bad proportions, background objects, full body, feet, shoes, boots, legs below thigh, pale skin, fair skin, light skin, white skin, hat, head wrap, headwrap, turban, crown, headdress, headpiece";

const MERRICK_BASE = "masterpiece, best quality, absurdres, highres, anime style, visual novel sprite, game cg, transparent background, png, white background, simple background, solo, 1girl, upper body, cowboy shot, dark skin, dark-skinned female, very dark brown skin, chocolate skin, african american, creole woman, striking emerald green eyes, glowing green eyes, long flowing black hair, hair past waist, simple elegant black off-shoulder dress, bare shoulders, gold chain necklace, small jade pendant, gold hoop earrings, subtle cleavage, confident mysterious smile, tall statuesque graceful, large breasts, looking at viewer, both hands visible, supernatural beauty, warm lighting, soft shading, clean lineart";
const SEED = 191919;

const OUTFITS = [
  { id: "body-casual", prompt: "casual clothes, oversized dark sweater, comfortable, relaxed", seed: 1 },
  { id: "body-formal", prompt: "elegant red evening gown, slit dress, gold jewelry, sophisticated", seed: 2 },
  { id: "body-school", prompt: "japanese school uniform, sailor collar, navy pleated skirt", seed: 3 },
  { id: "body-school-skimpy", prompt: "revealing school uniform, short skirt, unbuttoned shirt, loose tie", seed: 4 },
  { id: "body-cheerleader", prompt: "cheerleader outfit, crop top, pleated skirt, energetic", seed: 5 },
  ...
  { id: "body-vampire", prompt: "vampire queen, elegant gothic gown, deep v neckline, cape, choker, fangs", seed: 9 },
  ...
  { id: "body-bikini-front", prompt: "gold bikini, string bikini, swimsuit, beach, confident pose", seed: 14 },
  { id: "body-bikini-back", prompt: "from behind, looking back over shoulder, gold bikini, back view", seed: 15 },
];
...
function txt2img(positive, negative, w, h, seed) {
  return { prompt: {
    ...
    "7": { class_type: "SaveImage", inputs: { images: ["6", 0], filename_prefix: `merrick-full-${Date.now()}` } },
  }};
}
...
async function main() {
  console.log("=== Merrick Full Regen (body + 15 outfits + 15 expressions) ===\n");
  mkdirSync(SPRITES, { recursive: true });

  console.log("--- body-neutral (txt2img) ---");
  await gen(txt2img(MERRICK_BASE, NEG, 512, 768, SEED), join(SPRITES, "body-neutral.png"), "body-neutral");

  const baseName = "merrick-full-base.png";
  copyFileSync(join(SPRITES, "body-neutral.png"), join(COMFY_INPUT, baseName));

  console.log("\n--- outfits (img2img, denoise 0.55) ---");
  for (const outfit of OUTFITS) {
    await gen(
      img2img(`${MERRICK_BASE}, ${outfit.prompt}`, NEG, baseName, 512, 768, SEED + outfit.seed, 0.55),
  ...
  for (const [filename, exprPrompt] of Object.entries(EXPRESSIONS)) {
    await gen(
      img2img(`${MERRICK_BASE}, ${exprPrompt}`, NEG, baseName, 512, 768, SEED + Object.keys(EXPRESSIONS).indexOf(filename) + 100, 0.45),
...
"`, { stdio: "inherit" });
  ...
  console.log("\n=== Done! 31 Merrick images + bg removed. ===");
}
