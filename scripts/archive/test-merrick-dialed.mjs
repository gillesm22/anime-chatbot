/**
 * Test: Merrick with dialed-down weights matching Suzuka's range.
 * Skin 1.7→1.3, accessories simplified.
 */
...
const POSITIVE = [
  ...
  // Skin — dialed to Suzuka range (was 1.7/1.5, now 1.3/1.2)
  "(dark brown skin:1.3), (brown skin:1.2), smooth skin",
  // Framing — same as before
...
function txt2imgWorkflow(positive, negative, seed) {
  return {
    prompt: {
      ...
      "7": { class_type: "SaveImage", inputs: { images: ["6", 0], filename_prefix: `merrick-dialed-${seed}` } },
    },
  };
}
...
async function main() {
  ...
  for (const s of seeds) {
    ...
    const outPath = join(OUTPUT_DIR, `dialed-${s}.png`);
    await downloadImage(saveNode.images[0].filename, outPath);
    console.log(`  Saved: dialed-${s}.png`);
  }
  console.log("\n=== Done. ===\n");
}
