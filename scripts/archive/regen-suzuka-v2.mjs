/**
 * Regenerate Suzuka (nao) v2: matching the cowgirl's curvy body type and soft art style.
 * Run: node scripts/regen-suzuka-v2.mjs
 */
...
const OUTFITS = [
  ...
  { id: "body-cheer-extreme", prompt: "micro cheerleader, sports bra, very short skirt, athletic", seed: 6 },
  ...
  { id: "body-nurse", prompt: "nurse uniform, white dress, nurse cap, stethoscope, caring", seed: 10 },
  ...
  { id: "body-bikini-front", prompt: "blue string bikini, swimsuit, shy pose, beach, cute", seed: 14 },
  { id: "body-bikini-back", prompt: "from behind, looking back over shoulder, blue bikini, back view", seed: 15 },
];

const EXPRESSIONS = {
  "face-happy": "happy warm smile, bright eyes, genuine joy, cute blush",
  "face-thinking": "thoughtful expression, eyes looking slightly up, contemplative, finger on chin",
  "face-surprised": "surprised expression, wide eyes, slightly open mouth, cute shock",
  ...
  "face-angry": "angry expression, furrowed brows, intense glare, pout",
  "face-flustered": "flustered embarrassed expression, deep blush on cheeks, averted gaze, steam",
  "face-devoted": "deeply loving tender expression, warm soft gaze, gentle blush, adoring smile",
  "face-teasing": "playful teasing expression, sly grin, one eyebrow raised, mischievous, tongue out slightly",
  "face-sleepy": "drowsy sleepy expression, half-closed eyes, peaceful, yawning",
  "face-excited": "excited enthusiastic expression, wide sparkling eyes, big bright grin, fists up",
  "face-shy": "shy bashful expression, looking away, deep blush, hand near face, hiding",
  "face-jealous": "jealous expression, narrowed eyes, side glance, slight pout, annoyed",
...
async function main() {
  console.log("=== Suzuka v2 Full Regen (cowgirl body type) ===\n");
  mkdirSync(SPRITES, { recursive: true });

  console.log("--- body-neutral (txt2img) ---");
  await gen(txt2img(SUZUKA_BASE, NEG, 512, 768, SEED), join(SPRITES, "body-neutral.png"), "body-neutral");

  const baseName = "suzuka-v2-base.png";
  copyFileSync(join(SPRITES, "body-neutral.png"), join(COMFY_INPUT, baseName));

  console.log("\n--- outfits (img2img, denoise 0.55) ---");
  for (const outfit of OUTFITS) {
    await gen(
      img2img(`${SUZUKA_BASE}, ${outfit.prompt}`, NEG, baseName, 512, 768, SEED + outfit.seed, 0.55),
  ...
  for (const [filename, exprPrompt] of Object.entries(EXPRESSIONS)) {
    await gen(
      img2img(`${SUZUKA_BASE}, ${exprPrompt}`, NEG, baseName, 512, 768, SEED + Object.keys(EXPRESSIONS).indexOf(filename) + 100, 0.45),
...
"`, { stdio: "inherit" });
  ...
  console.log("\n=== Done! 31 Suzuka v2 images + bg removed. ===");
}
