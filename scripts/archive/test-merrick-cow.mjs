/**
 * Merrick cow outfit — V2 standard, max exposure.
 */
...
const POSITIVE = [
  ...
  "pretty face, mature face, (seductive smile:1.2)",
  "mature female, adult, (tall:1.2), (large breasts:1.2), slender waist, (curvy:1.2)",
  // Outfit — cow print, max exposure, casual terms
  "(cow print micro bikini:1.4), (cow pattern:1.3), cow horns headband, golden bell choker, bare midriff, bare thighs, maximum skin",
  // Accessories
  "gold hoop earrings",
  // Pose — asymmetric
...
function txt2imgWorkflow(positive, negative, seed) {
  return {
    prompt: {
      ...
      "7": { class_type: "SaveImage", inputs: { images: ["6", 0], filename_prefix: `merrick-cow-${seed}` } },
    },
  };
}
...
async function main() {
  ...
  console.log(`\n=== MERRICK COW OUTFIT (V2 Standard) ===\n`);

  // 5 seeds from proven range
  const seeds = [55600, 55630, 55650, 55660, 55670];
  for (const s of seeds) {
    ...
    const outPath = join(OUTPUT_DIR, `cow-${s}.png`);
    await downloadImage(saveNode.images[0].filename, outPath);
    console.log(`  Saved: cow-${s}.png`);
  }
  console.log("\n=== Done. ===\n");
}
