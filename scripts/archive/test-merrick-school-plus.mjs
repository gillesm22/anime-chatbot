/**
 * Merrick school+ (skimpy) outfit — V2 standard.
 */
...
const POSITIVE = [
  ...
  "pretty face, mature face, (confident smirk:1.2)",
  "mature female, adult, (tall:1.2), (large breasts:1.2), slender waist, (curvy:1.2)",
  // Outfit — school skimpy
  "(unbuttoned white shirt:1.3), (front knot shirt:1.2), (very short plaid skirt:1.3), bare midriff, bare thighs, thigh highs",
  // Accessories
  "gold hoop earrings",
  // Pose
...
function txt2imgWorkflow(positive, negative, seed) {
  return {
    prompt: {
      ...
      "7": { class_type: "SaveImage", inputs: { images: ["6", 0], filename_prefix: `merrick-schoolplus2-${seed}` } },
    },
  };
}
...
async function main() {
  ...
  const seeds = [55553, 55554, 55600, 55630, 55650, 55660, 55670, 55680, 55690, 55700];
  for (const s of seeds) {
    ...
    const outPath = join(OUTPUT_DIR, `schoolplus2-v2-${s}.png`);
    await downloadImage(saveNode.images[0].filename, outPath);
    console.log(`  Saved: schoolplus-v2-${s}.png`);
  }
  console.log("\n=== Done. ===\n");
}
