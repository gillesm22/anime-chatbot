/**
 * Test: Remove jewelry/accessory tags. Keep everything else from dialed-down.
 * Also drop (lace trim:1.2) — lingerie catalog association.
 */
...
const POSITIVE = [
  ...
  // Outfit — simplified, no lace trim, no jewelry
  "(black camisole:1.4), (cleavage:1.2)",
...
function txt2imgWorkflow(positive, negative, seed) {
  return {
    prompt: {
      ...
      "7": { class_type: "SaveImage", inputs: { images: ["6", 0], filename_prefix: `merrick-noacc-${seed}` } },
    },
  };
}
...
async function main() {
  ...
  console.log(`\n=== MERRICK NO-ACCESSORIES TEST ===`);
  console.log(`Removed: gold necklace, jade pendant, gold hoop earrings, lace trim, silk`);
  ...
  for (const s of seeds) {
    ...
    const outPath = join(OUTPUT_DIR, `noacc-${s}.png`);
    await downloadImage(saveNode.images[0].filename, outPath);
    console.log(`  Saved: noacc-${s}.png`);
  }
  console.log("\n=== Done. ===\n");
}
