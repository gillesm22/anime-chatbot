/**
 * Merrick vampire outfit — V2 standard, one render.
 */
...
const POSITIVE = [
  ...
  "(cowboy shot:1.5), (thighs visible:1.5), (show from head to mid-thigh:1.4), (head space:1.4), looking at viewer",
  "(long black hair:1.6), (hair past waist:1.3), straight hair",
  "(emerald green eyes:1.5), (long eyelashes:1.2)",
  "pretty face, mature face, (elegant expression:1.2), (small fangs:1.1)",
  "mature female, adult, (tall:1.2), (large breasts:1.2), slender waist, (curvy:1.2)",
  // Outfit — vampire, matching Suzuka's style
  "(vampire:1.3), (black corset mini dress:1.3), (bat wings:1.2), (garter stockings:1.2), (red ribbon accents:1.2), black choker with gem",
  // Accessories
  "gold hoop earrings",
  // Pose — asymmetric
...
function txt2imgWorkflow(positive, negative, seed) {
  return {
    prompt: {
      ...
      "7": { class_type: "SaveImage", inputs: { images: ["6", 0], filename_prefix: "merrick-vamp-55670" } },
    },
  };
}
...
async function main() {
  ...
  console.log(`\n=== MERRICK VAMPIRE (seed 55670) ===\n`);

  const seeds = [55660, 55670];
  for (const s of seeds) {
    ...
    const outPath = join(OUTPUT_DIR, `vamp-${s}.png`);
    await downloadImage(saveNode.images[0].filename, outPath);
    console.log(`  Saved: vamp-${s}.png`);
  }
  console.log("\n=== Done. ===\n");
}
