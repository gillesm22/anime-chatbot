/**
 * Test B: Remove (pale skin:1.4), (fair skin:1.4) from negative.
 * Keep everything else from the dialed-down version (solo:1.7, skin:1.3).
 */
...
// REMOVED: (pale skin:1.4), (fair skin:1.4)
const NEGATIVE = [
...
function txt2imgWorkflow(positive, negative, seed) {
  return {
    prompt: {
      ...
      "7": { class_type: "SaveImage", inputs: { images: ["6", 0], filename_prefix: `merrick-nopale-${seed}` } },
    },
  };
}
...
async function main() {
  ...
  console.log(`\n=== MERRICK NO-PALE TEST ===`);
  console.log(`Positive (${POSITIVE.length} chars)`);
  console.log(`Negative (${NEGATIVE.length} chars) — NO pale/fair skin tags\n`);
  ...
  for (const s of seeds) {
    ...
    const outPath = join(OUTPUT_DIR, `nopale-${s}.png`);
    await downloadImage(saveNode.images[0].filename, outPath);
    console.log(`  Saved: nopale-${s}.png`);
  }
  console.log("\n=== Done. ===\n");
}
