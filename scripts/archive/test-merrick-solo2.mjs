/**
 * Test C: solo bumped to 1.9/2.0, everything else same as dialed-down.
 */
...
const POSITIVE = [
  ...
  // Solo guards bumped: 1.7→1.9, 1girl 1.7→1.9, one person 1.5→1.7
  "(white background:1.4), (simple background:1.3), (no background:1.2), (solo:1.9), (1girl:1.9), (one person:1.7)",
...
function txt2imgWorkflow(positive, negative, seed) {
  return {
    prompt: {
      ...
      "7": { class_type: "SaveImage", inputs: { images: ["6", 0], filename_prefix: `merrick-solo2-${seed}` } },
    },
  };
}
...
async function main() {
  ...
  for (const s of seeds) {
    ...
    const outPath = join(OUTPUT_DIR, `solo2-${s}.png`);
    await downloadImage(saveNode.images[0].filename, outPath);
    console.log(`  Saved: solo2-${s}.png`);
  }
  console.log("\n=== Done. ===\n");
}
