/**
 * Merrick cheer-extreme outfit using V2 standard.
 * Tank top base identity, swap outfit only, asymmetric pose.
 */
...
const POSITIVE = [
  ...
  // Outfit — cheer extreme, casual terms, no lingerie words
  "(tiny sports bra:1.3), (micro pleated skirt:1.3), bare midriff, bare thighs",
  // Accessories — plain tags
...
function txt2imgWorkflow(positive, negative, seed) {
  return {
    prompt: {
      ...
      "7": { class_type: "SaveImage", inputs: { images: ["6", 0], filename_prefix: `merrick-cheer-${seed}` } },
    },
  };
}
...
async function main() {
  ...
  console.log(`\n=== MERRICK CHEER+ (V2 Standard) ===`);
  console.log(`Positive (${POSITIVE.length} chars)\n`);

  const seeds = [55553, 55600, 55630, 55650, 55660, 55670, 55680, 55690, 55700, 55710];
  for (const s of seeds) {
    ...
    const outPath = join(OUTPUT_DIR, `cheer-${s}.png`);
    await downloadImage(saveNode.images[0].filename, outPath);
    console.log(`  Saved: cheer-${s}.png`);
  }
  console.log("\n=== Done. ===\n");
}
