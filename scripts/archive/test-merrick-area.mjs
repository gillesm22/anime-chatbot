/**
 * Test: ConditioningSetAreaPercentage — restrict positive conditioning to center 80% of canvas.
 * Uses basic CLIPTextEncode (correct identity) + spatial restriction (no room for extras).
 * Dialed-down weights from the proven config.
 */
...
function txt2imgWorkflow(positive, negative, seed) {
  return {
    prompt: {
      ...
      // Restrict positive conditioning to center 80% of canvas
      "20": { class_type: "ConditioningSetAreaPercentage", inputs: {
        conditioning: ["2", 0],
        width: 0.80,
        height: 0.90,
        x: 0.10,
      ...
      "5": { class_type: "KSampler", inputs: {
        ...
        model: ["1", 0], positive: ["20", 0], negative: ["3", 0], latent_image: ["4", 0],
      }},
      "6": { class_type: "VAEDecode", inputs: { samples: ["5", 0], vae: ["1", 2] } },
      "7": { class_type: "SaveImage", inputs: { images: ["6", 0], filename_prefix: `merrick-area-${seed}` } },
    },
  };
}
...
async function main() {
  ...
  console.log(`\n=== MERRICK AREA CONDITIONING TEST ===`);
  console.log(`Center 80% width, 90% height (x:0.10, y:0.05)`);
  ...
  for (const s of seeds) {
    ...
    const outPath = join(OUTPUT_DIR, `area-${s}.png`);
    await downloadImage(saveNode.images[0].filename, outPath);
    console.log(`  Saved: area-${s}.png`);
  }
  console.log("\n=== Done. ===\n");
}
