/**
 * Test #3: dpmpp_2m (no sde) — deterministic sampler, no stochastic noise.
 * Everything else is the proven dialed-down config.
 */
  ...
  return {
    prompt: {
      ...
      "5": { class_type: "KSampler", inputs: {
        seed, steps: 40, cfg: 6.5,
        sampler_name: "dpmpp_2m",  // <-- no sde
        scheduler: "karras", denoise: 1,
        model: ["1", 0], positive: ["2", 0], negative: ["3", 0], latent_image: ["4", 0],
      }},
      "6": { class_type: "VAEDecode", inputs: { samples: ["5", 0], vae: ["1", 2] } },
      "7": { class_type: "SaveImage", inputs: { images: ["6", 0], filename_prefix: `merrick-nosde-${seed}` } },
    },
  };
}
...
async function main() {
  ...
  console.log(`\n=== MERRICK NO-SDE SAMPLER TEST (dpmpp_2m) ===\n`);
  ...
  for (const s of seeds) {
    ...
    const outPath = join(OUTPUT_DIR, `nosde-${s}.png`);
    await downloadImage(saveNode.images[0].filename, outPath);
    console.log(`  Saved: nosde-${s}.png`);
  }
  console.log("\n=== Done. ===\n");
}
