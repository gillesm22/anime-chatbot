/**
 * Merrick school outfit — V2 standard.
 */
...
const POSITIVE = [
  ...
  "(japanese school uniform:1.3), (white shirt:1.3), unbuttoned, cleavage, (plaid skirt:1.3), loose tie, thigh highs",
  // Accessories
  "gold hoop earrings",
  // Pose
...
function txt2imgWorkflow(positive, negative, seed) {
  return {
    prompt: {
      ...
      "7": { class_type: "SaveImage", inputs: { images: ["6", 0], filename_prefix: `merrick-school-${seed}` } },
    },
  };
}
...
async function main() {
  ...
  for (const s of seeds) {
    ...
    const outPath = join(OUTPUT_DIR, `school-v2-${s}.png`);
    await downloadImage(saveNode.images[0].filename, outPath);
    console.log(`  Saved: school-v2-${s}.png`);
  }
  console.log("\n=== Done. ===\n");
}
